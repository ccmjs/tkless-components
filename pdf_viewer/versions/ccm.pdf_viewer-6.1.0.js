...
( () => {
  ...
  const component = {
    ...
    config: {
      ...
      "css": [ "ccm.load", {  "context":"head", "url": "https://cdnjs.cloudflare.com/ajax/libs/font-awesome/4.7.0/css/font-awesome.min.css" },
    ...
    Instance: function () {
      ...
      const renderDescription = ( page ) => {
        const elem =  $.html( this.html.description, {
          description: () => this.description[ page ]
        } );

        this.element.querySelector( '#nav' ).appendChild( elem );
      };

      const touchEventHandling = () => {
