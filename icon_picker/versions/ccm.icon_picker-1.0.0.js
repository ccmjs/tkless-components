/**
 * @overview ccm component of icon picker for "Material Icons"
  ...
  const component = {
    ...
    Instance: function () {
      ...
      this.start = async () => {
        ...
        const input = self.element.querySelector( '.use-material-icon-picker' );
        input.before( $.html( '<i class="material-icons material-icon-picker-prefix prefix"></i>'));
        input.previousElementSibling.innerHTML = input.value;

        let event = new Event('keyup change');
        ...
        let picker = $.html('<div class="material-icon-picker" tabindex="-1" style="display: none"></div>');
        let search_input = $.html('<input id="search" type="text" placeholder="Search...">');

        // Simple filtering
        search_input.addEventListener( 'keyup', function () {
          let search = search_input.value.toLowerCase().trim();
          let icons = this.nextElementSibling;
          icons.querySelectorAll( 'i' ).forEach( i => {
            i.style.display = null;
            if( ! i.textContent.includes( search ) && search !== null )
        ...
        let icons = $.html('<div class="icons"></div>');
        material_icons.forEach( icon => {
          let $icon = $.html('<i class="material-icons"></i>');
          $icon.innerHTML = icon;
          $icon.addEventListener( 'click', function ( ) {
            input_value = this.innerHTML;
            this.closest( '.material-icon-picker' ).previousElementSibling.value = input_value;
            this.closest( '.material-icon-picker' ).previousElementSibling.dispatchEvent( event );
          } );
          icons.appendChild( $icon );
        });

        picker.appendChild( icons );
        self.element.querySelector( '.input-field' ).appendChild( picker );
