  ...
  const component = {
    ...
    Instance: function () {
      ...
      this.start = async () => {
        ...
        if ( self.editable && !self.user.isLoggedIn() ) await self.user.login();

        // get dataset for rendering
        dataset = await $.dataset( self.data );
        if ( !dataset.posts ) dataset.posts = [];

        if ( !dataset.posts && !self.editable )
        ...
        function renderPost( post, i )  {
          ...
          const post_elem = $.html( self.templates.post, {
            delete_post: () => {
