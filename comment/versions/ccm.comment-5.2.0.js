  ...
  const component = {
    ...
    Instance: function () {
      ...
      async function renderComments() {
        ...
        if ( self.voting && self.sorting_by_voting )
        ...
        unsorted_comments.map(  entry => {
          ...
          self.chat ? $.append( main_elem.querySelector( '#comment-list' ), entry.comment ) : $.prepend( main_elem.querySelector( '#comment-list' ), entry.comment );
        });
        ...
        async function renderComment( comment ) {
          ...
          const comment_elem = $.html( ( self.template === 'expanded' ) ? self.html.expanded: self.html.simple,
            {
              comment_content: comment.content,
              user: comment.user,
              date: self.chat ? moment( comment.date ).format( "MM Do, h:mm a" ) : moment( comment.date ).fromNow()
            } );

          // editable comments
          if ( self.editable ) {

            let edit_elem = $.html( self.html.edit, {
              edit: async () => {
                const instance = await self.editor.start( { root: comment_elem.querySelector( '.comment-overview' ) } );
      ...
      async function renderEditor() {
        ...
        const editor_elem = $.html( self.html.editor, {
          submit_button_label: self.submit_button_label ? self.submit_button_label: 'Senden',
      ...
      async function newComment() {
        ...
        if ( self.voting ) new_data.voting = data.key + '_' + ( data.comments.length + 1 );
